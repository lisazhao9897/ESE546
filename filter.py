...
def writeNum2(Num):
    with open("annotations/COCO_val"+year+".json","a+") as f:
        f.write(str(Num))
...
with open("annotations/instances_val"+year+".json","r+") as f:
    allData = json.load(f)
    data = allData["annotations"]
...
for i in data:
    if(i['category_id'] in classNum):
        ...
        inputfile2.append(inner2)
inputfile2 = json.dumps(inputfile2)
writeNum2(inputfile2)