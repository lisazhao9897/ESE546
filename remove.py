...
with open("annotations/COCO_val"+year+".json","r+") as f:
    data = json.load(f)

if year == '2014':
    for i in data:
        imgName = "COCO_val2014_" + str(i["filename"]).zfill(12) + ".jpg"
        nameStr2.append(imgName)
elif year == '2017':
    for i in data:
        imgName = str(i["filename"]).zfill(12) + ".jpg"
        nameStr2.append(imgName)

nameStr2 = set(nameStr2)

path = "val"+year+"/"
for file in os.listdir(path):
    if(file not in nameStr2):
        os.remove(path+file)